...
// Solo Enemy Awareness: player-count-keyed dampening of enemy DETECTION, so a solo player faces
// less "laser-focused" enemies. A Harmony Postfix on Enemy.Awake scales public tuning fields on
// each freshly-spawned enemy's EnemyVision component by the configured Detection multiplier for
// the current player count. 1.0 = vanilla; lower = gentler; 0 = effectively blind. At 4+ players
// the multiplier defaults to 1.0, so the feature naturally no-ops in full lobbies.
//
...
public static class SoloEnemyAwarenessPatch
{
    ...
    public static void Postfix(Enemy __instance)
    {
        try
        {
            ...
            Plugin.Log.LogDebug(
                $"[SoloEnemyAwareness] {enemyName} players={playerCount} det={detectionMult:F2} | " +
        ...
        {
            Plugin.Log.LogWarning($"[SoloEnemyAwareness] Postfix threw: {ex.GetType().Name}: {ex.Message}");
        }
    }
